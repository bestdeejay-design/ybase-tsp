...
foreach ($cities as $city) {
    ...
    // Определяем лимит на категорию по весу города
    $cityWeight = $db->query("SELECT weight FROM cities WHERE id = {$city['id']}")->fetchColumn();
    
    if ($cityWeight >= 500000) {
        $parseLimitPerCategory = 30; // Миллионники: 30 на категорию
    } elseif ($cityWeight >= 400000) {
        $parseLimitPerCategory = 25; // 400k-499k: 25 на категорию
    } elseif ($cityWeight >= 300000) {
        $parseLimitPerCategory = 20; // 300k-399k: 20 на категорию
    } elseif ($cityWeight >= 200000) {
        $parseLimitPerCategory = 15; // 200k-299k: 15 на категорию
    } elseif ($cityWeight >= 100000) {
        $parseLimitPerCategory = 10; // 100k-199k: 10 на категорию
    } else {
        $parseLimitPerCategory = 5;  // < 100k: 5 на категорию
    }
    ...
    foreach ($cats as $cat) {
        ...
        // Парсим заведения с учётом оставшегося лимита
        $list = [];
        $attempts = 0;
        $maxAttempts = 3;
        
        while (empty($list) && $attempts < $maxAttempts) {
            $list = $parser->parseList($city['slug'], $cat['yell'], $parseLimitPerCategory);
            if (empty($list)) {
                $attempts++;
